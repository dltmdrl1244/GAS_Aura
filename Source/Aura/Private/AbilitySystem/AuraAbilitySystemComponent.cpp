...
void UAuraAbilitySystemComponent::UpdateAbilityStatus(int32 Level)
{
	...
	{
		if (Info.AbilityTag.IsValid() == false || Level < Info.LevelRequirement)
		{
			continue;
		}
	...
	const FGameplayTag& Slot)
{
	...
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		...
		const bool bStatusValid = Status == GameplayTags.Abilities_Status_Equipped ||
			Status == GameplayTags.Abilities_Status_Unlocked;
		if (bStatusValid == true)
		{
			// Remove input tag(slot) from any ability that has it
...
void UAuraAbilitySystemComponent::ClientUpdateAbilityStatus(const FGameplayTag& AbilityTag,
